...
const appConfigObj: AppConfig = {
	...
	uploaderProvider: (process.env.UPLOADER_PROVIDER?.trim() ||
		"uploadthing") as "uploadthing",
	imageFieldNames: (
		process.env.IMAGE_FIELD_NAMES ||
		process.env.IMAGE_FIELD_NAME ||
		"image"
	)
		.split(",")
		.map((s) => s.trim())
		.filter(Boolean),
	itemNameFieldPath:
		process.env.ITEM_NAME_FIELD_PATH?.trim() || "fieldData.name",
	imageObjectFieldPath:
		process.env.IMAGE_OBJECT_FIELD_PATH?.trim() || "fieldData.image",
	concurrency: parseNumber(process.env.CONCURRENCY, 6),
	compression: {
		targetRatio: parseFloatNumber(
			process.env.COMPRESSION_TARGET_RATIO,
			0.5,
		),
		quality: parseNumber(process.env.COMPRESSION_QUALITY, 93),
		minQuality: parseNumber(process.env.COMPRESSION_MIN_QUALITY, 88),
		minSSIM: parseFloatNumber(process.env.COMPRESSION_MIN_SSIM, 0.98),
		maxDimension: parseNumber(process.env.COMPRESSION_MAX_DIMENSION, 1920),
	},
};
